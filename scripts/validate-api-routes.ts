...
async function validateApiRoutes() {
  ...
  for (const file of routeFiles) {
    ...
    if (info.issues.length > 0) {
      ...
      console.log(`   Auth: ${info.hasAuth ? '✓' : '✗'} | Error Handling: ${info.hasErrorHandling ? '✓' : '✗'} | CORS: ${info.hasCORS ? '✓' : '✗'}`)
  ...
  console.log(`  Authentication: ${withAuth}/${results.length} (${Math.round(withAuth/results.length*100)}%)`)
  console.log(`  Error Handling: ${withErrorHandling}/${results.length} (${Math.round(withErrorHandling/results.length*100)}%)`)
  console.log(`  CORS Headers: ${withCORS}/${results.length} (${Math.round(withCORS/results.length*100)}%)`)
  console.log()
