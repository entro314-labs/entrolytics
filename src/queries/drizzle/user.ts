...
export async function getUsers(
	...
	const query = db
		...
		.leftJoin(
			website,
			and(eq(user.userId, website.userId), isNull(website.deletedAt)),
		)
		.where(and(...conditions))
		.groupBy(
			user.userId,
...
export async function deleteUser(userId: string) {
	...
	return db.transaction(async (tx) => {
		...
		await tx
			.delete(sessionData)
			.where(inArray(sessionData.websiteId, websiteIds));
		await tx
			.delete(websiteEvent)
			.where(inArray(websiteEvent.websiteId, websiteIds));
		await tx.delete(session).where(inArray(session.websiteId, websiteIds));
		await tx
			.delete(orgUser)
			.where(or(inArray(orgUser.orgId, orgIds), eq(orgUser.userId, userId)));
		await tx.delete(org).where(inArray(org.orgId, orgIds));
		await tx
			.delete(report)
			.where(
				or(inArray(report.websiteId, websiteIds), eq(report.userId, userId)),
			);
