...
const originalPath = path.resolve(
	process.cwd(),
	".next/routes-manifest-orig.json",
);
...
if (collectApiEndpoint) {
	const apiRoute = originalManifest.headers.find(
		(route) => route.source === API_PATH,
	);
...
if (trackerScriptName) {
	const trackerRoute = originalManifest.headers.find(
		(route) => route.source === TRACKER_SCRIPT,
	);
...
if (rewrites.length !== 0) {
	...
	const builtHeaders = headers.map((header) =>
		buildCustomRoute("header", header),
	);
	const builtRewrites = rewrites.map((rewrite) =>
