...
async function hasAdminUsers() {
	try {
		...
		console.error(
			colorize("❌ Error checking for admin users:", "red"),
			error.message,
		);
		return false;
	}
...
async function getTotalUserCount() {
	try {
		...
		console.error(
			colorize("❌ Error getting total user count:", "red"),
			error.message,
		);
		return 0;
	}
...
async function getUserByEmail(email) {
	try {
		...
		console.error(
			colorize("❌ Error finding user by email:", "red"),
			error.message,
		);
		return null;
	}
...
async function promoteUserToAdmin(email) {
	try {
		...
		if (!existingUser) {
			...
			console.log(
				colorize(
					"💡 Make sure the user has signed up through Clerk first",
					"yellow",
				),
			);
			return false;
		...
		console.log(
			colorize(`✅ Successfully promoted ${email} to admin`, "green"),
		);
		...
		console.error(
			colorize(`❌ Error promoting user ${email} to admin:`, "red"),
			error.message,
		);
		return false;
	}
...
async function listAdminUsers() {
	try {
		...
		console.log(
			colorize(`📋 Found ${adminUsers.length} admin user(s):`, "green"),
		);
		console.log("");

		adminUsers.forEach((admin, index) => {
			console.log(
				colorize(`${index + 1}. ${admin.displayName || "Unnamed"}`, "bright"),
			);
		...
		console.error(
			colorize("❌ Error listing admin users:", "red"),
			error.message,
		);
	}
}
...
async function showStatus() {
	try {
		...
		console.log(
			`${colorize("Has Admin Users:", "blue")} ${hasAdmins ? colorize("✅ Yes", "green") : colorize("❌ No", "red")}`,
		);

		console.log("");
		console.log(colorize("Configuration:", "bright"));
		console.log(
			`${colorize("INITIAL_ADMIN_EMAIL:", "blue")} ${config.initialAdminEmail || colorize("Not set", "yellow")}`,
		);
		console.log(
			`${colorize("AUTO_PROMOTE_FIRST_USER:", "blue")} ${config.autoPromoteFirstUser ? colorize("Enabled", "green") : colorize("Disabled", "yellow")}`,
		);
		...
		if (!hasAdmins) {
			...
			if (config.initialAdminEmail) {
				console.log(
					colorize(
						`💡 Users with email ${config.initialAdminEmail} will be auto-promoted to admin`,
						"cyan",
					),
				);
			}
			...
			if (config.autoPromoteFirstUser) {
				console.log(
					colorize(
						"💡 The first user to sign up will be auto-promoted to admin",
						"cyan",
					),
				);
			}
			...
			if (!config.initialAdminEmail && !config.autoPromoteFirstUser) {
				console.log(
					colorize(
						"💡 Set INITIAL_ADMIN_EMAIL or AUTO_PROMOTE_FIRST_USER to enable auto-promotion",
						"cyan",
					),
				);
			}
...
function validateConfig() {
	...
	if (!config.initialAdminEmail && !config.autoPromoteFirstUser) {
		errors.push(
			"Either INITIAL_ADMIN_EMAIL or AUTO_PROMOTE_FIRST_USER should be configured",
		);
	}
...
async function main() {
	...
	if (!process.env.DATABASE_URL) {
		console.error(
			colorize("❌ DATABASE_URL environment variable is required", "red"),
		);
		process.exit(1);
	}
