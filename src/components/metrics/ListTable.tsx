...
export function ListTable({
	...
	const getRow = (row: ListData, index: number) => {
		...
		const safeLabel =
			typeof label === "string" ? label : String(label || "unknown");
		...
		return (
			<AnimatedRow
				...
				label={
					renderLabel
						? renderLabel(row, index)
						: safeLabel || formatMessage(labels.unknown)
				}
				value={count}
				percent={percent}
	...
	const ListTableRow = ({
		...
		const safeLabel =
			typeof rowData?.label === "string"
				? rowData.label
				: String(rowData?.label || "unknown");
