...
export function WebsiteTransferForm({
	...
	const { mutate, error, isPending } = useUpdateQuery(
	...
	const items =
		orgs?.data?.filter(({ orgUser }) =>
			orgUser.find(
				({ role, userId }) =>
					[ROLES.orgOwner, ROLES.orgManager].includes(role) &&
					userId === user.id,
			),
		) || [];
	...
	return (
		<Form onSubmit={handleSubmit} error={error} values={{ orgId }}>
			<Text>
				{formatMessage(
					isOrgWebsite
						? messages.transferOrgWebsiteToUser
						: messages.transferUserWebsiteToOrg,
				)}
			</Text>
