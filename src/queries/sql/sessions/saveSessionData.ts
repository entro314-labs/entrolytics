...
import { CLICKHOUSE, DRIZZLE, db, runQuery, sessionData as sessionDataTable } from '@/lib/db';
...
export async function relationalQuery({
  ...
  const existing = await db
    ...
    .where(
      and(eq(sessionDataTable.websiteId, websiteId), eq(sessionDataTable.sessionId, sessionId)),
    );
