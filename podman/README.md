# How to deploy entrolytics on podman
...
3. Start entrolytics by running `podman-compose up -d`.

If you need to stop entrolytics, you can do so by running `podman-compose down`.


### Install systemd service (optional)

If you want to install a systemd service to run entrolytics, you can use the provided
systemd service.

Edit `entrolytics.service` and change these two variables:


	WorkingDirectory=/opt/apps/entrolytics
	EnvironmentFile=/opt/apps/entrolytics/.env
...
that entrolytics is not currently running.  If you started it previously, bring it
down using:
