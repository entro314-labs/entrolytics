...
export async function hasPermission(
	...
	try {
		...
		const hasPermissionFromRole = ensureArray(permission).some(
			(perm) =>
				platformPermissions.includes(perm) ||
				platformPermissions.includes(PERMISSIONS.all),
		);
		...
		if (orgId && orgRole) {
			const orgRolePermissions =
				ROLE_PERMISSIONS[
					`org${orgRole.charAt(0).toUpperCase()}${orgRole.slice(1)}`
				] || [];
			return ensureArray(permission).some((perm) =>
				orgRolePermissions.includes(perm),
			);
		}
...
export async function getAuthContext() {
	try {
		...
		const platformRole =
			(clerkUser.publicMetadata?.role as string) || ROLES.user;
