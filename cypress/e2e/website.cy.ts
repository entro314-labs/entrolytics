describe("Website tests", () => {
	...
	it("Edit a website", () => {
		...
		cy.getDataTest("input-name")
			.find("input")
			.type("Updated website", { delay: 0 });
		cy.getDataTest("input-domain").find("input").click();
		cy.getDataTest("input-domain").find("input").clear();
		cy.getDataTest("input-domain")
			.find("input")
			.type("updatedwebsite.com", { delay: 0 });
		cy.getDataTest("button-submit").click({ force: true });
		cy.getDataTest("input-name")
			.find("input")
			.should("have.value", "Updated website");
		cy.getDataTest("input-domain")
			.find("input")
			.should("have.value", "updatedwebsite.com");
		...
		cy.get("textarea").should(
			"contain.text",
			Cypress.config().baseUrl + "/script.js",
		);
	...
	it("Delete a website", () => {
		...
		cy.contains(/Type DELETE in the box below to confirm./i).should(
			"be.visible",
		);
