...
import { API_ENDPOINTS, CLI_CONFIG, getEnvVarNames } from '@entro314labs/shared-constants';
...
describe('CLI to SDK Integration', () => {
  ...
  describe('CLI Token-based Setup', () => {
    it('should accept --token flag', async () => {
      const { stdout } = await execAsync(`npx @entro314labs/entro-cli init --token ${testToken}`, {
        cwd: testDir,
      });
    ...
    it('should create .env file with correct variables', async () => {
      ...
      await execAsync(`npx @entro314labs/entro-cli init --token ${testToken} --framework nextjs`, {
        cwd: testDir,
      });
    ...
    it('should validate token before setup', async () => {
      ...
      try {
        await execAsync(`npx @entro314labs/entro-cli init --token ${invalidToken}`, {
          cwd: testDir,
        });
    ...
    it(
      ...
      20 * 60 * 1000,
    ); // 20 minute timeout for this test
  });
...
`;
    ...
    it('should initialize React SDK with CLI-generated config', async () => {
      await execAsync(`npx @entro314labs/entro-cli init --token ${testToken} --framework react`, {
        cwd: testDir,
      });
    ...
    it('should initialize Vue SDK with CLI-generated config', async () => {
      await execAsync(`npx @entro314labs/entro-cli init --token ${testToken} --framework vue`, {
        cwd: testDir,
      });
  ...
  describe('Event Ingestion', () => {
    it('should track events with SDK after CLI setup', async () => {
      // Setup with CLI
      await execAsync(`npx @entro314labs/entro-cli init --token ${testToken} --framework node`, {
        cwd: testDir,
      });
  ...
  describe('Framework Detection', () => {
    ...
    it('should detect Svelte from svelte.config.js', async () => {
      await fs.writeFile(path.join(testDir, 'svelte.config.js'), 'export default {}');
    ...
    it('should detect Astro from astro.config.mjs', async () => {
      await fs.writeFile(path.join(testDir, 'astro.config.mjs'), 'export default {}');
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', async () => {
      ...
      try {
        await execAsync(`npx @entro314labs/entro-cli init --token ${testToken}`, { cwd: testDir });
  ...
  describe('Onboarding Integration', () => {
    it('should mark onboarding as complete after successful setup', async () => {
      ...
      const response = await fetch(`${API_ENDPOINTS.production}/api/user/onboarding`);
