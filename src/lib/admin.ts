...
import { getUserByEmail, getUsers, updateUser } from '@/queries'
...
export async function shouldPromoteToAdmin(userEmail: string): Promise<boolean> {
  ...
  if (config.initialAdminEmail && normalizedEmail === config.initialAdminEmail) {
    console.log(`🔐 Auto-promoting user with email ${userEmail} to admin (matches INITIAL_ADMIN_EMAIL)`)
    return true
  }
  ...
  if (config.autoPromoteFirstUser) {
    ...
    if (!hasAdmins && totalUsers <= 1) {
      console.log(`🔐 Auto-promoting first user ${userEmail} to admin (AUTO_PROMOTE_FIRST_USER enabled)`)
      return true
    }
  }
 ...
 */
export function validateAdminSetupConfig(): { valid: boolean; errors: string[] } {
  const config = getAdminSetupConfig()
  const errors: string[] = []
  ...
  if (!config.initialAdminEmail && !config.autoPromoteFirstUser) {
    errors.push('Either INITIAL_ADMIN_EMAIL or AUTO_PROMOTE_FIRST_USER should be configured to ensure admin access')
  }
