...
describe('CLI Framework Detection', () => {
  ...
  describe('Next.js Detection', () => {
    it('should detect Next.js from package.json dependencies', () => {
      ...
      const hasNext = FRAMEWORK_PACKAGES.nextjs.some(pkg => packageJson.dependencies[pkg]);
    ...
    it('should prioritize Next.js over React', () => {
      ...
      const hasNext = FRAMEWORK_PACKAGES.nextjs.some(pkg => packageJson.dependencies[pkg]);
      const hasReact = FRAMEWORK_PACKAGES.react.some(pkg => packageJson.dependencies[pkg]);
  ...
  describe('Vue Detection', () => {
    it('should detect Vue from package.json', () => {
      ...
      const hasVue = FRAMEWORK_PACKAGES.vue.some(pkg => packageJson.dependencies[pkg]);
    ...
    it('should detect Vite-based Vue app', () => {
      ...
      const hasVue = FRAMEWORK_PACKAGES.vue.some(
  ...
  describe('Svelte Detection', () => {
    it('should detect Svelte from package.json', () => {
      ...
      const hasSvelte = FRAMEWORK_PACKAGES.svelte.some(pkg => packageJson.dependencies[pkg]);
    ...
    it('should detect Vite-based Svelte app', () => {
      ...
      const hasSvelte = FRAMEWORK_PACKAGES.svelte.some(
        pkg => packageJson.dependencies[pkg] || packageJson.devDependencies?.[pkg],
      );
  ...
  describe('Astro Detection', () => {
    it('should detect Astro from package.json', () => {
      ...
      const hasAstro = FRAMEWORK_PACKAGES.astro.some(pkg => packageJson.dependencies[pkg]);
    ...
    it('should detect frameworks from config files', () => {
      ...
      for (const [file, framework] of Object.entries(configFileMap)) {
        ...
        if (pattern?.configFiles) {
          expect(pattern.configFiles.some((f: string) => f.includes(file.split('.')[0]))).toBe(
            true,
          );
        }
      }
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle devDependencies', () => {
      ...
      const hasNext = FRAMEWORK_PACKAGES.nextjs.some(pkg => packageJson.devDependencies[pkg]);
