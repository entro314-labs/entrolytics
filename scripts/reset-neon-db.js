...
async function resetDatabase() {
  ...
  try {
    ...
    if (tables.length > 0) {
      console.log(`📊 Found ${tables.length} tables to drop:`, tables.map(t => t.tablename).join(', '))
    ...
    const migrationPath = path.join(__dirname, '../src/lib/db/migrations/0001_complete_schema_migration.sql')
