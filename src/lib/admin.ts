 ...
 */
export async function shouldPromoteToAdmin(
	userEmail: string,
): Promise<boolean> {
	...
	if (
		...
		normalizedEmail === config.initialAdminEmail
	) {
		console.log(
...
export async function promoteUserToAdmin(userId: string): Promise<boolean> {
	try {
		...
		console.error(
			`❌ Failed to promote user ${userId} to admin - user not found`,
		);
		return false;
	} catch (error) {
...
export async function promoteUserToAdminByEmail(
	...
	try {
		...
		console.error(
			`❌ Error promoting user with email ${email} to admin:`,
			error,
		);
		return false;
	}
 ...
 */
export async function bootstrapAdminSetup(
	userEmail: string,
	userId: string,
): Promise<void> {
	try {
...
export async function getAdminSetupStatus() {
	...
	return {
		...
		totalUsers,
		setupComplete:
			hasAdmins || config.initialAdminEmail || config.autoPromoteFirstUser,
	};
}
